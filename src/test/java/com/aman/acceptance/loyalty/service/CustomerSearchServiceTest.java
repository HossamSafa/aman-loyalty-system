...
class CustomerSearchServiceTest {
    ...
    void setUp() {
        customerSearchService =
                new CustomerSearchService(customerRepository, mobileUtil);
    }
    ...
    void search_whenInputLooksLikeMobile_shouldSearchByHash() {
        ...
        PagedResponse<?> result =
                customerSearchService.search("+201012345678", pageable);
    ...
    void search_whenInputIsName_shouldSearchByName() {
        ...
        PagedResponse<?> result =
                customerSearchService.search("Sara", pageable);

        assertThat(result.getItems()).hasSize(1);

        verify(customerRepository)
                .findByNameContainingIgnoreCase(eq("Sara"), any());

        verify(customerRepository, never())
                .findByMobileHash(anyString());
    }
